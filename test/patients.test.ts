...
import { PatientsModel, PersonsModel, UsersModel } from "../src/infrastructure/db";
import type { PatientsModelInstance } from "../src/services/patients/infrastructure/data/patients.types";
...
const basePatientModel = (): PatientsModelInstance =>
  ({
    pat_id: 1,
    per_id: 1,
    pat_sta_state: 1,
...
describe("POST /patients", () => {
  it("creates patient for admin", async () => {
    vi.spyOn(PersonsModel, "findByPk").mockResolvedValue(basePersonModel());
    vi.spyOn(PatientsModel, "findOne").mockResolvedValue(null as PatientsModelInstance | null);
    vi.spyOn(PatientsModel, "create").mockResolvedValue(basePatientModel());

    const res = await request(app).post("/patients").set("Authorization", "Bearer admin").send({
      per_id: 1,
      pat_sta_state: 1,
    });

    expect(res.status).toBe(201);
    expect(res.body.pat_id).toBe(1);
  });

  it("allows receptionist to create patient", async () => {
    vi.spyOn(PersonsModel, "findByPk").mockResolvedValue(basePersonModel());
    vi.spyOn(PatientsModel, "findOne").mockResolvedValue(null as PatientsModelInstance | null);
    vi.spyOn(PatientsModel, "create").mockResolvedValue(basePatientModel());
  ...
  it("returns 403 when role is not allowed", async () => {
    const res = await request(app).post("/patients").set("Authorization", "Bearer other").send({
      per_id: 1,
      pat_sta_state: 1,
    });
